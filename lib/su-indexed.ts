...
export const suIndexed: GetSoupUtilFn = ((
  soup: AnyCircuitElement[],
  options: SoupUtilOptions = {},
) => {
        ...
        return {
          get: (id: string) => {
            ...
            if (indexConfig.byId && internalStore.indexes.byId) {
              return internalStore.indexes.byId.get(`${component_type}:${id}`) as Extract<AnyCircuitElement, { type: typeof component_type }> || null
            }
            ...
            if (indexConfig.byType && internalStore.indexes.byType) {
              ...
              ) as Extract<AnyCircuitElement, { type: typeof component_type }> || null
            }
          ...
          getUsing: (using: any) => {
            ...
            if (indexConfig.byRelation && internalStore.indexes.byRelation) {
              ...
              if (relationMap) {
                ...
                const joinerId = joiner[`${component_type}_id` as keyof typeof joiner]
                
                if (indexConfig.byId && internalStore.indexes.byId) {
                  return internalStore.indexes.byId.get(`${component_type}:${joinerId}`) as Extract<AnyCircuitElement, { type: typeof component_type }> || null
                }
          ...
          getWhere: (where: any) => {
            ...
            if (indexConfig.byType && internalStore.indexes.byType) {
              ...
              return elementsOfType.find(
                (e: any) => keys.every(key => e[key] === where[key])
              ) as Extract<AnyCircuitElement, { type: typeof component_type }> || null
            }
          ...
          list: (where?: any) => {
            ...
            if (keys.length === 0 && indexConfig.byType && internalStore.indexes.byType) {
              return (internalStore.indexes.byType.get(component_type) || []) as Extract<AnyCircuitElement, { type: typeof component_type }>[]
            }
            
            // If filtering by subcircuit_id and it's the only filter
            if (keys.length === 1 && keys[0] === 'subcircuit_id' && 
                indexConfig.bySubcircuit && internalStore.indexes.bySubcircuit) {
              const subcircuitId = where.subcircuit_id
              const subcircuitElements = internalStore.indexes.bySubcircuit.get(subcircuitId) || []
            ...
            return elementsToFilter as Extract<AnyCircuitElement, { type: typeof component_type }>[]
          },
            ...
            if (indexConfig.byRelation && internalStore.indexes.byRelation) {
              ...
              for (const [key, value] of elementEntries) {
                if (key.endsWith("_id") && key !== `${newElm.type}_id` && typeof value === "string") {
                  const relationTypeMap = internalStore.indexes.byRelation.get(key) || new Map()
                  const relatedElements = relationTypeMap.get(value as string) || []
          ...
          delete: (id: string) => {
            ...
            if (indexConfig.byId && internalStore.indexes.byId) {
              ...
              const elementsOfType = internalStore.indexes.byType.get(component_type) || []
              elm = elementsOfType.find((e: any) => e[`${component_type}_id`] === id)
            } else {
            ...
            if (indexConfig.byRelation && internalStore.indexes.byRelation) {
              ...
              for (const [relationKey, relationMap] of internalStore.indexes.byRelation.entries()) {
          ...
          update: (id: string, newProps: any) => {
            ...
            if (indexConfig.bySubcircuit && internalStore.indexes.bySubcircuit && 
                'subcircuit_id' in elm && 'subcircuit_id' in newProps) {
            ...
            if (indexConfig.byCustomField && internalStore.indexes.byCustomField) {
              for (const field of indexConfig.byCustomField) {
                if (field in elm && field in newProps && (elm as any)[field] !== newProps[field]) {
                  const fieldMap = internalStore.indexes.byCustomField.get(field)
                  if (fieldMap) {
            ...
            if (indexConfig.byCustomField && internalStore.indexes.byCustomField) {
              for (const field of indexConfig.byCustomField) {
                if (field in elm && field in newProps) {
                  const fieldValue = (elm as any)[field]
                  if (fieldValue !== undefined && (typeof fieldValue === "string" || typeof fieldValue === "number")) {
                    const fieldValueStr = String(fieldValue)
                    const fieldMap = internalStore.indexes.byCustomField.get(field)!
                    const elementsWithFieldValue = fieldMap.get(fieldValueStr) || []
            ...
            return elm as Extract<AnyCircuitElement, { type: typeof component_type }>
          },
          ...
          select: (selector: string) => {
            ...
            if (component_type === "source_component") {
              ...
              if (component_type === "source_port") return source_port as Extract<AnyCircuitElement, { type: typeof component_type }>
